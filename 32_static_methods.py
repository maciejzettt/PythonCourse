...
class Cake:
    ...
    def save_to_file(self, fPath, fName):
        fName = fName + ".bakery"
        if os.path.isdir(fPath):
            try:
                file = open(os.path.join(fPath, fName), 'bw')
                pickle.dump(self, file)
                file.close()
                print("{0} - successfully saved to file.".format(self.name))
            except:
    ...
    @classmethod
    def read_from_file(cls, fPath, fName):
        fName = fName + ".bakery"
        if os.path.isfile(os.path.join(fPath, fName)):
            try:
                file = open(os.path.join(fPath, fName), 'br')
                newObj = pickle.load(file)
                file.close()
                Cake.bakery_offer.append(newObj.name)
                return newObj
            except:
                print("Error during file handling.")
        else:
    ...
    def list_files(fPath):
        files = glob.glob(os.path.join(fPath, "*.bakery"))
        print("Bakery files in {0}".format(fPath))
        for file in files:
            print("\t{0}".format(file))


Cake.list_files("G:/py/exdata")
